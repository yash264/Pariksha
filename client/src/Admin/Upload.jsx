...
function Upload() {
    ...
    const getPdf = async () => {
        const result = await axios.get('http://localhost:3000/getFiles');
        setPdf(result.data.data[0].UploadedFiles);
    };

    const showPdf = (pdf) => {
        window.open(`http://localhost:3000/files/${pdf}`, "_blank", "noreferror");
    }

    const deletePdf = (pdf) => {
        axios.post('http://localhost:3000/deletePdf', {pdf})
        .then(result=>{
    ...
    const submitPdf = async (e) => {
        ...
        const result = await axios.post('http://localhost:3000/uploadPdf', formData, { headers: { "Content-Type": "multipart/form-data" } });
